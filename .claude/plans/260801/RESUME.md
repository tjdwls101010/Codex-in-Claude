...
> **역할을 다한 문서다.** v0.2.0은 2026-08-02에 릴리즈됐다. 아래 내용은 그 시점의 중단 상태이며 지금은 전부 지나간 것이다 — 현재 상태는 `.claude/harness-spec.md`에 있다. 남겨두는 이유는 하나뿐이다: 세션이 컨텍스트를 잃고 이어질 때 무엇을 적어둬야 실제로 이어지는지의 사례로서다.
