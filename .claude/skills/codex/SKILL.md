...
**One run or a batch.** A batch is this skill's fan-out: N runs under one name, so watching, collecting and stopping are each one call instead of N. Reach for it the moment you would otherwise be tracking several run ids by hand. What it costs is N base-instruction floors, where N turns on one thread pay one floor plus that growing replay — so parallel wins when the work is genuinely independent, and one thread wins when each step needs what the last one learned.

**If you run Claude Code's dynamic workflows, you already know the shape.** A batch is that instrument with another vendor's agents inside it, and the correspondence is close enough to plan against: `--task` repeated is `parallel()`; `--as-ready` under `--resume-from` is `pipeline()` rather than a barrier; `--schema` is a stage's `schema`, with one asymmetry at collection time that its own help names; `--worktree` is `isolation: 'worktree'`; and a `--tasks-file` entry can set `model` and `effort` per member the way `agent()` takes them per call. This is the one thing about a batch that no `--help` here will ever tell you, because no command of this CLI knows that workflows exist — and a caller who misses it prices a batch as bookkeeping.
...
**Where the correspondence stops is the part that decides things.** A workflow is a program, so between its stages arbitrary code runs: round two's prompts can be *computed* from round one's results — deduplicated against everything seen so far, filtered by a verdict, repeated until a round comes back empty. `--resume-from` pairs positionally, and what crosses the phase boundary is each thread's own memory rather than a value you calculated from all of them. So reach for a batch when every round can be written down before the one before it returns, and for a workflow when it cannot.
