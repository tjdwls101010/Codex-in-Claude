...
| B23 | Ask one question of a whole group's state, and be notified when the *group* ends | skill+script | `status --group <name> [--follow]` | validated | T1 `GroupSelectors` (9 tests) incl. `group.completed` / `group.still-running` terminal lines, so a group can never end in silence |
| B24 | Concurrent writers must not edit each other's files mid-edit | script | per-member git worktree at `.codex-runs/<run_id>/wt` | validated | V-13 (main tree stays clean; `git worktree remove` refuses a dirty tree); T1 `Assignment` (13 tests). Per member, not per batch: V-15 measured that a fresh worktree has 0 lines of `git diff HEAD`, so `read-only` and `review` members must stay in the caller's tree |
| B25 | Collect a group's results under a cap, and surface what more than one member wrote | skill+script | `result --group`, `overlaps` | validated | T1 `GroupResults` (6 tests) incl. cutting and measuring in the same unit. `overlaps` is the intersection only (D30) and is keyed by run, so a resumed member never overlaps its own past self |
...
| V-17 | **deferred to M4a** | `status --group --follow` does not exist yet, so there is nothing to pair with Monitor. It reuses `log --follow`'s loop, whose Monitor pairing is already validated (B21); the group-specific check belongs to the milestone that builds it. |
