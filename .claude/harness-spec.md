...
- **`--max-concurrent` / N above 8.** V-11 and V-12 measured up to N=8 and no higher. A 4-member batch's spawn was timed at 1.45 s and a 5-member one ran clean in a headless session, so the sequential spawn loop is not the bottleneck at those sizes, but N=12/16/24 has not been run. D34 still makes `--stagger` conditional on contention that has never been observed.
- **Batch collection cost.** `GROUP_MESSAGE_CAP = 4000` still has "looks reasonable" as its justification. `docs/measurements/filter-calibration.md`'s method has not been applied to `result --group` across N.
- **`projected_cost` against reality.** It degrades honestly when there are fewer than three isolated completed runs to sample (observed), but the predicted floor has never been compared with what a batch actually cost.
