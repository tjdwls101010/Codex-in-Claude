...
**A loop that verifies has to end on a predicate the work can satisfy.** "Until nothing comes back" is not one: a sufficiently new lens finds something in any codebase, and this repository measured that — four adversarial rounds, not one of them at zero. Counting findings is a stopping rule that never stops, because the quantity is generated by the lens rather than drawn from a finite set. What ended it here was counting a *different* quantity: of the nineteen findings, three were reachable in ordinary single-session use, and that is a property of the work rather than of the examiner, so it runs out. Ask for an enumeration, a named property, a JSON answer over a set you can bound — `--schema`'s own help states what `result` then hands back and how it fails.

**`resume` or a fresh `start`.** A resumed thread keeps what it already worked out and replays a transcript that grows every turn — measured on one thread, input tokens roughly doubled per turn for the first few. A fresh thread pays Codex's base-instruction floor once and knows nothing. Continue when the new work depends on the old understanding; start fresh when it does not, because the replay is not free and an unrelated task inherits a context it has to read past. Judge it per thread; there is no threshold worth memorising. In a loop that verifies, the replay is the cheaper half of this: a fresh round also does not know what you rejected, so it reopens the questions the last one closed — and "still wrong" and "asked again" reach you as one list of the same shape. A thread this skill never started — one left behind in the Codex TUI — is a third option, reachable by its id; what `resume` requires before it will touch one is in `resume --help`.
...
*And whether anything has to be computed between rounds.* If every round's prompts can be written down before the round before it returns, `--resume-from` carries them — and a phase boundary you did not need costs each member a full turn of that replay. If round two's prompts come *from* round one's results — deduplicated against everything seen so far, filtered by a judgement you made, repeated until a round comes back empty — that is a program, and it runs in your own context between two batches, or in a workflow.
