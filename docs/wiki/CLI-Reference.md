...
`start` and `resume` additionally share:
...
| `--add-dir <path>` | An extra writable root, beyond `--cwd` (repeatable). Only available on a fresh `start` — `resume` can't add one later |
...
$CODEX batch start --group <name> (--task "<prompt>"... | --tasks-file <jsonl>)
                   ...
                   [--force]
                   [any start/resume flag as a group-wide default]
```
...
| `--tasks-file <path>` | JSONL, one task object per line. Fields: `prompt`, `kind`, `label`, `model`, `effort`, `sandbox`, `schema`, `image`, `cwd`, `resume` |
