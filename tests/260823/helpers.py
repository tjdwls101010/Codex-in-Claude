"""Scaffolding for the 260823 round.
...
`BridgeCase` here is a copy of the 260814 round's rather than an import of it.
Every round names its scaffolding `helpers`, and `unittest discover` puts its own
start directory on `sys.path` first — so an import across rounds resolves to
whichever suite is being run, which is a failure that would look like a test bug
...
class BridgeCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.tmp = Path(tempfile.mkdtemp(prefix="codex-260823-")).resolve()
        ...
        self.env["CLAUDE_CODE_SESSION_ID"] = "test-260823"
