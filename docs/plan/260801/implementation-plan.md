...
**상태:** 구현 완료, v0.2.0으로 릴리즈됨(2026-08-02). 2026-08-01 계획 전용 세션에서 작성.

> **이 문서를 지금 읽는 사람에게.** 계획은 대체로 맞았지만 일곱 곳이 틀렸고, 전부 코드를 읽어서가 아니라 **실제로 돌려서** 드러났다. 어느 절을 근거로 삼기 전에 `.claude/harness-spec.md`의 **R10~R16**을 먼저 읽어라 — 무엇이 왜 틀렸는지가 거기 있다. 특히 §3.2("실패는 그 자리에 기록된다"), §3.3(Monitor 페어링), §3.4(overlaps를 run 단위로), §3.6(`batch clean`의 거부 조건)은 **그대로 따르면 안 된다.** D01~D37 자체는 유효하다; 틀린 것은 그 결정들을 어떻게 실현하는지에 대한 절들이다.
>
