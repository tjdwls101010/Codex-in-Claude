...
| A run vanished when the session ended | Session-end cleanup ran, which is the intended default behavior | `resume` it — a SIGINT-stopped thread is resumable. Use `--detach` next time if it should outlive the session. See [Session Cleanup Hook](Session-Cleanup-Hook.md) |
| A background run is still going and nothing knows about it | A `--detach`ed run has outlived its session | `status --all` finds it; `stop --run <id>` when you're done with it. `doctor` lists these under `detached_running` |
