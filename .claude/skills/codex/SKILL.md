...
**The idiom, the moment `start` comes back:** put `log --run <id> --follow --level compact` in a **background Bash** call. It exits on the run's last line, and a background Bash call that exits notifies you — which is the contract the Agent tool gives you for a subagent, and what "Codex as a managed subagent" was always supposed to mean. *Then* decide what to do with the turn. Genuinely parallel work, do it. No parallel work, and another turn coming, hand the turn back armed and say so — but check that second half first, because handing back an armed turn is exactly what the one-turn case below cannot do. Do not invent work to fill the wait either, which spends tokens nobody asked for and, in its most obvious form, walks into the concurrent-writer gotcha below. Arming costs nothing and is safe at any moment: aimed at a run that is already over it prints the last line and exits at once.
...
**Exception — this is your only turn.** If you cannot name the thing that would wake you, this is a one-turn context, and arming is useless whichever instrument you reach for: a background follower and a Monitor both die with the turn. Do any parallel work first, then make the turn's *last* call a blocking foreground `--follow` — and **raise that Bash call's own `timeout`** — it is in milliseconds, defaults to 120,000 and caps at 600,000 — and bound the follow just under it with `--follow-timeout`. Measured: of four sessions given this shape, the one that passed is the one that raised the Bash timeout, and the three that left it at its default were cut off after two minutes and reported that they were still watching. If you also need to *intervene* mid-run, Monitor cannot help you here either — it wakes a turn you do not get — so the shape is a foreground `log --since <cursor>` loop, each pass short enough to act between passes.
...
**Isolating writers is decided when a group is started, and cannot be decided later.** A resumed run takes its directory from its thread and there is no `--worktree` on `resume`, so `batch start --worktree --resume-from` cuts nothing either. Three threads that were started sharing a tree — the default — go on sharing it whichever command continues them; three that were given checkouts keep those. Measured in an e2e session, which escaped only because the three edits happened to land in three different files. You do not have to spot it yourself — a writing run started into a directory another live writing run already occupies comes back with `concurrent_writers` naming them, and `doctor` reports the same across the registry — but that report arrives after the spawn, and the decision is before it.
